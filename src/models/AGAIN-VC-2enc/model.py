...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for block in self.conv1d_blocks:
            y = block(y)
            h, mn, sd = self.inorm(y, return_mean_std=True)
            mns.append(mn)
            sds.append(sd)
            y = y + h
...
class SpeakerEncoder(nn.Module):
    def __init__(
        ...
        for _, sub in zip(range(n_conv_blocks), subsample):
            self.conv1d_blocks.append(
                EncConvBlock(c_h, c_h, subsample=sub, normalize=False)
            )
    ...
    def forward(self, x):
        ...
        for conv, linear in zip(self.conv1d_blocks, self.linear_blocks):
            y = conv(y)
        
        y = rearrange(self.pooling_layer(y), 'b c 1 -> b c')

        for linear in self.linear_blocks:
            h = linear(y)
            mn, sd = h[:, :self.c_h], h[:, self.c_h:]
            mns.append(rearrange(mn, 'b c -> b c 1'))
            sds.append(rearrange(sd, 'b c -> b c 1'))
